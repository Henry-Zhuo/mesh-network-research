...
/**
 * \ingroup tcpclientserver
 * Create TcpTraceClient application which sends UDP packets based on a trace
 ...
 */
class TcpTraceClientHelper
{
  public:
    /**
     * Create TcpTraceClientHelper which will make life easier for people trying
     * to set up simulations with tcp-client-server.
     *
     */
    TcpTraceClientHelper();
     ...
     */
    TcpTraceClientHelper(Address ip, uint16_t port, std::string filename);
    /**
     * Create TcpTraceClientHelper which will make life easier for people trying
     * to set up simulations with tcp-client-server. Use this variant with
     * addresses that do include a port value (e.g., InetSocketAddress and
     ...
     * \param filename the file from which packet traces will be loaded
     */
    TcpTraceClientHelper(Address addr, std::string filename);
