...
TypeId
TcpEchoServer::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::TcpEchoServer")
            ...
            .AddTraceSource("Rx",
                            ...
                            MakeTraceSourceAccessor(&TcpEchoServer::m_rxTrace),
                            "ns3::Packet::TracedCallback")
            .AddTraceSource("RxWithAddresses",
                            "A packet has been received",
                            MakeTraceSourceAccessor(&TcpEchoServer::m_rxTraceWithAddresses),
...
void
TcpEchoServer::StartApplication()
{
    NS_LOG_FUNCTION(this);
    ...
    if (!m_socket)
    {
        ...
        if (addressUtils::IsMulticast(m_local))
        {
            ...
            if (tcpSocket)
            {
                ...
                tcpSocket->MulticastJoinGroup(0, m_local);
            }
            else
            {
    ...
    if (!m_socket6)
    {
        ...
        {
            Ptr<TcpSocket> tcpSocket = DynamicCast<TcpSocket>(m_socket6);
            if (tcpSocket)
            {
                // equivalent to setsockopt (MCAST_JOIN_GROUP)
                tcpSocket->MulticastJoinGroup(0, local6);
            }
            else
            {
                NS_FATAL_ERROR("Error: Failed to join multicast group");
